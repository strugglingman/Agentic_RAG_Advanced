param(
    [string]$TemplateFile = "config/eks/eks_addon_ebs_csi_create.template.json",
    [string]$MapFile = "config/aws_deployment.local.map",
    [string]$OutputFile = "config/eks/eks_addon_ebs_csi_create.from_local.json",
    [switch]$Apply,
...
Write-Host "Rendered EBS CSI addon create JSON to: $resolvedOutput"
...
if ($LASTEXITCODE -eq 0 -and $existingAddonJson) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to create EBS CSI addon: $applyOutput"
    }
...
if ($WaitUntilActive) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "EBS CSI addon did not become ACTIVE."
    }
}
