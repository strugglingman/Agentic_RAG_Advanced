param(
    [string]$RoleName = "agentic-rag-eks-cluster-role",
    [string]$TrustPolicyFile = "config/iam/agentic_rag_eks_cluster_role_trust_policy.template.json",
    [string]$ManagedPolicyArn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
)
...
$outputPath = Join-Path $repoRoot "config/iam/eks_cluster_role.from_local.json"
