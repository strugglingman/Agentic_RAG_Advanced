param(
    [string]$RoleName = "agentic-rag-eks-cloudwatch-observability-role",
    [string]$TrustPolicyFile = "config/iam/agentic_rag_eks_cloudwatch_observability_role_trust_policy.template.json",
    [string]$ManagedPolicyArn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
)
...
$outputPath = Join-Path $repoRoot "config/iam/eks_cloudwatch_observability_role.from_local.json"
