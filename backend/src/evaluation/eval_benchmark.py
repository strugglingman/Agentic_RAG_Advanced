...
if not _openai_api_key:
    raise RuntimeError(
        "Missing OPENAI_API_KEY environment variable; set it before running eval_benchmark.py"
    )
...
def best_answer_score(
    norm_answer_bot: str, norm_gold_answers: list[str], score
) -> float:
    return max(
        (
            score(norm_answer_bot, norm_gold_answer)
            for norm_gold_answer in norm_gold_answers
        ),
        default=0.0,
    )
...
def eval(d: dict[str, any], k_list: list[int], mode: str):
    ...
    t0 = time.time()
    top_k = max(k_list)
    ctx_bot, err = rc.retrieve(
        query, top_k, use_reranker=use_rerank, use_hybrid=use_hybrid
    )
    ...
    norm_gold_answers = [
        " ".join(gold_answer.strip().lower().split())
    ...
    f1 = 0.0
    if any(
        norm_answer_bot == norm_gold_answer for norm_gold_answer in norm_gold_answers
    ):
        exact = 1
    elif any(
        norm_answer_bot in norm_gold_answer or norm_gold_answer in norm_answer_bot
        for norm_gold_answer in norm_gold_answers
    ):
        partial = 1
    else:
...
def main():
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to the dataset or using default data, with fields: query, gold_sources[], gold_answers[] (optional)",
    )
    p.add_argument(
        "--list-k",
        type=str,
        default="",
        help="List of K results to retrieve and evaluate hit@K",
    )
    p.add_argument(
        "--mode",
        choices=["sem", "hybrid", "rerank", "hybrid_rerank"],
        default="sem",
        help="Retrieval mode",
    )
    p.add_argument(
        "--output", type=str, default="", help="Path to save the detailed results"
    )
    args = p.parse_args()
    path_data = args.data
    list_k = (
        [int(k) for k in args.list_k.strip().split(",") if k.isdigit()]
        if args.list_k
        else [1, 3, 5, 10]
    )
    mode = args.mode
    ...
    with open(file_summary, "w", encoding="utf-8") as fsum:
        ...
        retrieval_metrics = {
            **{
                f"hit@{k}": mean(
                    r.get("retrieval_metrics", {}).get(f"hit@{k}", 0) for r in results
                )
                for k in list_k
            },
            **{
                f"mrr@{k}": mean(
                    r.get("retrieval_metrics", {}).get(f"mrr@{k}", 0.0) for r in results
                )
                for k in list_k
            },
            **{
                f"recall@{k}": mean(
                    r.get("retrieval_metrics", {}).get(f"recall@{k}", 0.0)
                    for r in results
                )
                for k in list_k
            },
            **{
                f"ndcg@{k}": mean(
                    r.get("retrieval_metrics", {}).get(f"ndcg@{k}", 0.0)
                    for r in results
                )
                for k in list_k
            },
